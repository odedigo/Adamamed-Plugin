...
class Adamamed_OptionsManager {
    ...
    public function settingsPage() {
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'adamamed'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'adamamed'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'adamamed'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'adamamed');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'adamamed'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'adamamed');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'adamamed'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'adamamed') ?>"/>
                </p>
            </form>
        </div>
        <?php
     ...
     * Stats page
     */
    public function adamamed_statsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'adamamed'));
        }
     ...
     * Google contacts - not active
     */    
    public function adamamed_manualOrdersPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'adamamed'));
        }

        $mo = new Adamamed_Manual_Order();
        $mo->drawPage($this);        
    }
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'adamamed');
            case 'false':
                return __('false', 'adamamed');

            case 'Administrator':
                return __('Administrator', 'adamamed');
            case 'Editor':
                return __('Editor', 'adamamed');
            case 'Author':
                return __('Author', 'adamamed');
            case 'Contributor':
                return __('Contributor', 'adamamed');
            case 'Subscriber':
                return __('Subscriber', 'adamamed');
            case 'Anyone':
                return __('Anyone', 'adamamed');
        }
        return $optionValue;
    }
